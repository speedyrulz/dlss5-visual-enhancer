...
class DLSSFrameSession:
    ...
    def reconcile_bound_adapter(self) -> None:
        """Verify the worker used the requested adapter; correct the report.

        The worker picks its own Direct3D adapter, so an explicit AI GPU
        selection it did not honor must fail fast instead of silently
        rendering on another card; automatic selection just fixes reporting.
        """
    ...
    def send_frame(
        ...
        *,
        ...
        if self._streaming and (
            index != self._sent_frames or self._sent_frames >= 0xFFFFFFFF
        ):
    ...
    def receive_frame(self, index: int, pts: int) -> tuple[np.ndarray, int]:
    ...
    def process(
        ...
        *,
        ...
        return self.receive_frame(index, pts)
